...
module.exports = async function (callback) {
   ...
   try {
        ...
        for(let i = 1; i <= 5; i++) {
            await exchange.makeOrder(hflToken.address, toWei(200 + i), ETHER_ADDRESS, toWei(i/1000), {from: accounts[0]});
            console.log(`account[0]制作第${i}个订单`);
            await wait(5)
        }

        for(let i = 1; i <= 5; i++) {
            await exchange.makeOrder(hflToken.address, toWei(2000 + i), ETHER_ADDRESS, toWei(i/100), {from: accounts[1]});
